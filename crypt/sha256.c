...
/* transformFunction
 * Compress 512-bits */
static void _transform(sha256_context_t *context,
                       uint8_t const *buf)
{
  uint32_t S[8];
  uint32_t W[64];
                   ...
                   uint32_t bufsize)
{
  ...
  while (bufsize > 0) {
    if (context->curlen == 0 && bufsize >= BLOCK_SIZE) {
      _transform(context, (uint8_t *)buf);
      context->length += BLOCK_SIZE * 8;
      buf = (uint8_t *)buf + BLOCK_SIZE;
      bufsize -= BLOCK_SIZE;
    }
    else {
      n = MIN(bufsize, (BLOCK_SIZE - context->curlen));
      memcpy_fast(context->buf + context->curlen, buf, (size_t)n);
      context->curlen += n;
      buf = (uint8_t *)buf + n;
      bufsize -= n;
      if (context->curlen == BLOCK_SIZE) {
        _transform(context, context->buf);
                     ...
                     sha256_hash_t *digest)
{
   ...
   * then compress. Then we can fall back to padding zeros and
                      ...
                      sha256_hash_t *digest)
{
  ...
  sha256_finalise(&context, digest);
}
