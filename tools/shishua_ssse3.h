/* CC0 1.0 Universal
 * written by Thaddée Tyl
 * https://github.com/espadrine/shishua */

#ifndef _SHISHUA_SSSE3_H_
#define _SHISHUA_SSSE3_H_
...
/* buf could technically alias with prng_state,
 * according to the compiler */
#define SHISHUA_RESTRICT __restrict


/* buf's size must be a multiple of 128 bytes */
static inline void prng_gen(prng_state_t *SHISHUA_RESTRICT s,
                            uint8_t *SHISHUA_RESTRICT buf,
                            size_t size)
{
  ...
  for (size_t i = 0; i < size; i += 128) {
    ...
    for (size_t j = 0; j < 2; j++) {
       ...
       * entangle bits with other 64-bit positions.
       * We must shift by an odd number so that each bit reaches all 64-bit
       * positions, not just half.
       * We must lose bits of information, so we minimize it: 1 and 3.
       * We use different shift values to increase divergence between
       * the two sides.
       ...
       * 64-bit additions to strong positions for enrichment.
       * The low 32-bit part of a 64-bit chunk never moves to the same
       * 64-bit chunk as its high part.
       * They do not remain in the same chunk.
       * Each part eventually reaches all positions ringwise:
       * A to B, B to C, …, H to A.
...
static uint64_t phi[16] = {
  ...
  0xF06AD7AE9717877E, 0x85839D6EFFBD7DC6,
  0x64D325D1C5371682, 0xCADD0CCCFDFFBBE1,
  0x626E33B8D04B4331, 0xBBF73C790D94F79D,
  0x471C4AB3ED3D82A5, 0xFEC507705E4AE6E5,
};

prng_state_t *prng_new()
{
  /* Note: output is uninitialized at first, but since we pass NULL,
   * its value is initially ignored */
  prng_state_t *s = xmalloc(sizeof(prng_state_t));
  return s;
}

void prng_delete(prng_state_t *s)
{
  xfree(s);
}

void prng_init(prng_state_t *s, SEEDTYPE seed[4])
{
